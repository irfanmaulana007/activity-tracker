...
import { QuickStartItem, UseQuickStartResult } from '~/types'
...
export const useQuickStart = (): UseQuickStartResult => {
  ...
  const getRecentProjects = (): QuickStartItem[] => {
    return quickStartItems.filter((item) => item.type === 'project').slice(0, 5)
  }

  const getRecentGames = (): QuickStartItem[] => {
    return quickStartItems.filter((item) => item.type === 'game').slice(0, 5)
  }
